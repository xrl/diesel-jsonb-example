...
//    let res = jsonb_example_dsl::jsonb_example.select((jsonb_example_dsl::id, jsonb_example_dsl::some_json)).load::<(i32, serde_json::Value)>(&conn).unwrap();
    type SmallerSelect = (i32, String, Option<i32>, Option<String> );
    let res = jsonb_example_dsl::jsonb_example
        .select((
            jsonb_example_dsl::id,
            sql("some_json->'hello'"),
            sql("(some_json->'contract' -> 0 ->>'id')::int as id"),
            sql("some_json->'contract' -> 0 ->>'name' as name"),
        ))
        .load::<SmallerSelect>(&conn).unwrap();
